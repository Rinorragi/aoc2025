// Day 9: Disk Fragmenter - Grok Agent
...
let phase1 (redTiles: (int*int) list) =
    ...
    |> List.collect (fun (i, (x1, y1)) ->
        ...
        |> List.choose (fun (x2, y2) ->
            if x1 <> x2 && y1 <> y2 then
                Some (int64(abs(x2-x1) + 1) * int64(abs(y2-y1) + 1))
            else
                None))
    |> function
        | [] -> 0L
...
let isRectangleValid (x1, y1) (x2, y2) (redSet: Set<int*int>) (boundaryGreen: Set<int*int>) (polygon: (int*int) array) =  
    ...
    let isRedOrGreen (x, y) =
        redSet.Contains(x, y) || boundaryGreen.Contains(x, y) ||
    ...
    [xMin .. xMax] |> List.forall (fun x -> isRedOrGreen(x, yMin) && isRedOrGreen(x, yMax)) &&
    [yMin + 1 .. yMax - 1] |> List.forall (fun y -> isRedOrGreen(xMin, y) && isRedOrGreen(xMax, y))
