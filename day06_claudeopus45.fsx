...
let solve (lines: string[]) =
    ...
    let colValues =
        colRanges
        |> List.map (fun (startIdx, endIdx) ->
            let op = opLine.[startIdx]
            let nums =
                dataLines
                |> Array.choose (fun line ->
                    if startIdx < line.Length then
                        let actualEnd = min endIdx (line.Length - 1)
                        let s = line.Substring(startIdx, actualEnd - startIdx + 1).Trim()
...
let validateExample () =
    ...
    let res2, ms2 = timeit solve_phase2 exampleLines
    if res1 = expected1 && res2 = expected2 then
        printfn "Example OK: Phase 1: %A (Time: %d ms), Phase 2: %A (Time: %d ms)" res1 ms1 res2 ms2
    else
...
let runPhases () =
    ...
    let part2, ms2 = timeit solve_phase2 inputLines
